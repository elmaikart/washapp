...
export default function CanastoRopaBlancaPage() {
  ...
  const [devoluciones, setDevoluciones] = React.useState<Franja[]>([
    { ...franjaBase },
  ]);
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <TimeBlock
                  ...
                  minMinuteIfMinHour={f.desdeMinuto !== "--" ? f.desdeMinuto : undefined}
                  disabled={f.confirmada}
                />
              ...
              <div className="flex items-center gap-2 whitespace-nowrap shrink-0">
                {!f.confirmada ? (
                  <button
                    title={canConfirm ? "Confirmar franja" : "Franja inválida (mínimo 1 h)"}
                    onClick={() => {
  ...
  /* ---------- VALIDACIÓN GLOBAL ---------- */
  const allRetirosOk =
    retiros.length > 0 && retiros.every((f) => f.confirmada && franjaValida1h(f));
  const allDevolucionesOk =
    devoluciones.length > 0 &&
    devoluciones.every((f) => f.confirmada && franjaValida1h(f));
  const anyCantidad = cantidades.some((c) => c > 0);

  const pedidoListo = allRetirosOk && allDevolucionesOk && anyCantidad;
            ...
            <div>
              ...
              <div className="flex items-center mb-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRetiros(retiros.map((r) => ({ ...r, fecha: e.target.value })))
                  }
                  className="border border-gray-300 rounded-md px-3 py-2 w-full"
                />
              </div>
              <RenderFranja tipo="retiro" franjas={retiros} setFranjas={setRetiros} />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center mb-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDevoluciones(
                      devoluciones.map((d) => ({ ...d, fecha: e.target.value }))
                    )
                  }
                  className="border border-gray-300 rounded-md px-3 py-2 w-full"
                />
